...
13. Quality gate: `cargo build && cargo test`. Then `cargo test <context_snake_case>::` — the slice's own tests are enough; don't run the whole suite each time.
...
- Example: this board context's own tag helper lives in fn wallet_tag()
```
...
- "Tests require DATABASE_URL pointing at a real Postgres for the integration suite"
...
**Read `.build-kit/CLAUDE.md` in full before your first slice.** It carries the
one rule `slice.json` doesn't state that matters most here — where DCB tags
come from — plus the `pii`/`sensitive_fields()` mapping and the "one Rust
module per Context, not per slice" file convention this stack uses instead of
the per-slice-folder convention some other stacks follow.

**There's no separate framework file to read first.** `skilj`/`skilj-core`
(published crates) already are the read/fold/decide/append plumbing other
stacks hand-roll — the only project-owned Rust to read before your first
slice is `src/wallet.rs`, the worked example.
...
`.build-kit/CLAUDE.md`, worked example at `docs/screens/EXAMPLE-wallet-balance.md` — and **stop**.

**Quality gate:** `cargo build && cargo test`. Fix it before committing — never
commit a crate that doesn't build clean.

**Write the pure `decide()`/`project()` unit test first, no Postgres needed**
— see `.build-kit/AGENTS.md`'s own note on why this is both faster and the
better-fitting default for this stack than skilj's own demo integration-test
style.
