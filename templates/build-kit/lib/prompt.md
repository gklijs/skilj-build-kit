...
3. Determine the **slice type** from the slice.json — see `.build-kit/CLAUDE.md`'s own "Building a slice" section for the exact field-presence rules this stack uses (and why `sliceType === "TRANSLATION"` / `queries`/`projections` field names aren't assumed to exist without checking a real export first):
...
5. Run quality checks (`cargo build && cargo test`, then the slice's own tests only — `cargo test <context_snake_case>::`, not the full test suite each time).
